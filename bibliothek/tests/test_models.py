...
class MediaModelTest(TestCase):
    ...
    def test_media_type_choices(self):
        media = Media.objects.create(title='Test Media', media_type='book', genre=Genre.objects.get(id=1), language=Language.objects.get(id=1), description='Test Description', release_year=2022)
        self.assertIn(media.media_type, dict(Media.MEDIA_CHOICES).keys())
    ...
    def test_genre_relationship(self):
        ...
        self.assertEquals(media.genre, genre)
    ...
    def test_language_relationship(self):
        ...
        self.assertEquals(media.language, language)
...
class MovieModelTest(TestCase):
    ...
    def test_director_relationship(self):
        ...
        self.assertEquals(movie.director, director)

    def test_imdb_label(self):
        movie = Movie.objects.create(title='Test Movie', media_type='movie', genre=Genre.objects.get(id=1), language=Language.objects.get(id=1), description='Test Description', release_year=2022, director=Director.objects.get(id=1), FSK=12, IMDb=7.5)
        field_label = movie._meta.get_field('IMDb').verbose_name
        self.assertEquals(field_label, 'IMDb')
...
class UserBorrowedModelTest(TestCase):
    ...
    def test_user_relationship(self):
        ...
        self.assertEquals(borrowed_item.user.username, 'testuser')

    def test_media_relationship(self):
        borrowed_item = UserBorrowed.objects.get(id=1)
        self.assertEquals(borrowed_item.media.title, 'Test Media')
...
class UserReservedModelTest(TestCase):
    ...
    def test_user_relationship(self):
        ...
        self.assertEquals(reserved_item.user.username, 'testuser')

    def test_media_relationship(self):
        reserved_item = UserReserved.objects.get(id=1)
        self.assertEquals(reserved_item.media.title, 'Test Media')

    def test_reserved_till_label(self):
        reserved_item = UserReserved.objects.get(id=1)
        field_label = reserved_item._meta.get_field('reserved_till').verbose_name
        self.assertEquals(field_label, 'reserved till')
...
class UserReviewModelTest(TestCase):
    ...
    def test_user_relationship(self):
        ...
        self.assertEquals(review.user.username, 'testuser')

    def test_media_relationship(self):
        review = UserReview.objects.get(id=1)
        self.assertEquals(review.media.title, 'Test Media')
